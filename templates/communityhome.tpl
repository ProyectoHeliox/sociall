
{{if $page}}
<div>{{$page}}</div>
{{/if}}

<h3 id="extra-help-header">Help or '@NewHere'?</h3>
<div id="extra-help">
<a href="https://helpers.pyxis.uberspace.de/profile/helpers"
	title="Friendica Support" target="_blank">Friendica Support</a><br />
<a href="https://letstalk.pyxis.uberspace.de/profile/letstalk"
	title="Let's talk" target="_blank">Let's talk</a><br />
<a href="http://newzot.hydra.uberspace.de/profile/newzot"
	title="Local Friendica" target="_blank">Local Friendica</a><br />
<a href="http://kakste.com/profile/newhere" title="@NewHere" target="_blank">NewHere</a>
</div>

<h3 id="connect-services-header">Connectable Services</h3>
<ul id="connect-services">
<li><a href="{{$url}}/facebook"><img alt="Facebook"
	src="view/theme/sociall/icons/facebook.png" title="Facebook" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="StatusNet" src="view/theme/sociall/icons/StatusNet.png" title="StatusNet" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="LiveJournal" src="view/theme/sociall/icons/livejournal.png" title="LiveJournal" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="Posterous" src="view/theme/sociall/icons/posterous.png" title="Posterous" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="Tumblr" src="view/theme/sociall/icons/tumblr.png" title="Tumblr" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="Twitter" src="view/theme/sociall/icons/twitter.png" title="Twitter" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="WordPress" src="view/theme/sociall/icons/wordpress.png" title="WordPress" /></a></li>
<li><a href="{{$url}}/settings/connectors"><img
	alt="E-Mail" src="view/theme/sociall/icons/email.png" title="E-Mail" /></a></li>
</ul>

